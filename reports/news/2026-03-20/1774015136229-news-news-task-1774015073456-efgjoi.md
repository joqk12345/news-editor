...
- agent: news/news
- requestId: 1774015073456-efgjoi
- 生成时间(UTC): 2026-03-20T13:58:56.228Z
...
graph TD
  subgraph "Concept Map"
    VI["视觉智能 / Visual Intelligence"] --> PE["视角 / Perspective"]
    VI --> BE["生物演化 / Biological Evolution"]
    VI --> TM["技术建模 / Technical Modeling"]
    VI --> AE["应用终局 / Application Endgame"]
    TM --> HR["层次化表征 / Hierarchical Representation"]
    TM --> PP["并行化处理 / Parallel Processing"]
    TM --> FS["特征共享 / Feature Sharing"]
    TM --> AC["抽象认知 / Abstract Cognition"]
    VI --> WM["世界模型 / World Model"]
    VI --> GR["通用机器人 / General-Purpose Robotics"]
    LLM["大语言模型 / Large Language Model"] --> DS["数字化空间 / Digital Space"]
    VI --> PW["物理世界 / Physical World"]
  end
```
### Mermaid 图 2（逻辑/因果图）
```mermaid
flowchart LR
  subgraph "Causal Chain"
    EV["视觉的诞生 / Emergence of Vision"] --> AR["军备竞赛 / Arms Race"]
    AR --> CE["寒武纪物种大爆发 / Cambrian Explosion"]
    CE --> IE["智能爆发 / Intelligence Explosion"]
    HR["层次化表征 / Hierarchical Representation"] --> AC["抽象认知 / Abstract Cognition"]
    PP["并行化处理 / Parallel Processing"] --> PW["物理世界交互 / Physical World Interaction"]
    VI["视觉智能 / Visual Intelligence"] --> WM["世界模型 / World Model"]
    VI --> GR["通用机器人 / General-Purpose Robotics"]
  end
```
