...
- agent: news/news
- requestId: 1774103587771-z1icb7
- 生成时间(UTC): 2026-03-21T14:34:27.519Z
...
graph TD
  subgraph "Concept Map"
    ...
    subgraph "历史阶段"
      direction LR
      H1["生理学起源 / Physiology Origin"]
      H2["控制理论应用 / Control Theory Application"]
      H3["强化学习理论 / RL Theory"]
      H4["深度学习架构 / DL Architecture"]
    end
    C1["Kenneth Craik"]
    C2["模型预测控制 / MPC"]
    C3["Dyna架构 / Dyna"]
    C4["JEPA / JEPA"]
    AR["抽象表征空间 / Abstract Representation Space"]
...
flowchart LR
  subgraph "Causal Chain"
    A["概念提出 (1943) / Concept Proposed"] --> B["工程验证 (60-70s) / Engineering Validation"]
    B --> C["理论升华 (Dyna) / Theoretical升华"]
    C --> D["架构复兴 (JEPA) / Architecture Revival"]
    D --> E["终极目标：自主智能 / Ultimate Goal: Autonomous Intelligence"]
    
    A1["Craik: 预测动作后果 / Predict Action Consequences"] --> A
    B1["MPC: 推演最优序列 / Rollout Optimal Sequence"] --> B
    C1["Sutton: 区分策略类型 / Distinguish Policy Types"] --> C
    D1["LeCun: 抽象表征预测 / Abstract Representation Prediction"] --> D
  end
```
