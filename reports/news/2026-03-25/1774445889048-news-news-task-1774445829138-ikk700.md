...
- agent: news/news
- requestId: 1774445829138-ikk700
- 生成时间(UTC): 2026-03-25T13:38:09.047Z
...
graph TD
  subgraph "Concept Map"
    ...
    C3["Transformer / Transformer architecture"]
    C4["XLA / XLA compiler"]
    C5["Gemini / Gemini model"]
    C1 --> C2
    C1 --> C3
    C4 --> C1
  end
...
flowchart LR
  subgraph "Causal Chain"
    T["TPU技术发展与商业应用 / TPU Technology Development and Commercial Application"]
    R["文章揭示了TPU的起源、苹果成为其最大用户的原因、Anthropic大额采购的深层关系，以及谷歌内部在TPU使用和研发上的矛盾与历史。"]
    A["苹果通过挖角获得TPU软件生态经验，成为最大用户。"]
    T --> R
    R --> A
  end
```
